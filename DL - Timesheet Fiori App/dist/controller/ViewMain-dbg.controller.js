sap.ui.define([
     ...
     */
    function (Controller, coreLibrary, Fragment, Filter, DateFormat, ValueState, MessageToast, MessageBox) {
        "use strict";
        return Controller.extend("zfioritimesheet3.zfioritimesheet3n.controller.ViewMain", {
            // Gestione primo caricamento (login), evento onBeforeRendering e pulsante per apertura manager view inizio
            onInit: function () { //Initialization.
                ...
                var flagModelExecuted = oJsonModel.getProperty("/flagExecuted");
                if (flagModelExecuted) {
                    //Set core to not busy after backend data fetching:
                    sap.ui.core.BusyIndicator.hide();
                    return false;
                }
                oJsonModel.setProperty("/flagExecuted", true);
                oJsonModel.refresh();
                //Fetch backend data:
                //UserDataSet:
                oModel.read("/UserDataSet('STEFANLU')", {
                    success: function (oDataUserData) {
                        var sPathToPersons = "/people",
                            aPersons = oJsonModel.getProperty(sPathToPersons),
                            oLoggedUser;
                        if (oDataUserData.FlgManager) {
                            oLoggedUser = {
                                key: oDataUserData.Uname,
                                key2: oDataUserData.Pernr,
                                title: oDataUserData.Uname,
                                text: oDataUserData.Unamex,
                                icon: "sap-icon://leads",
                                manager: true,
                                appointments: []
                            };
                        ...
                        aPersons.push(oLoggedUser);
                        if (!oJsonModel.setProperty(sPathToPersons, aPersons)) {
                            that.showLogInError();
                            return false;
                        }
                        //UserEventSet:
                            ...
                            oUserEventFilter = new Filter({
                                filters: [
                                    new Filter({
                                        ...
                                        value1: oDataUserData.Uname
                                    }),
                                    new Filter({
                                        path: "InputPernr",
                                        operator: sap.ui.model.FilterOperator.EQ,
                                        value1: oDataUserData.Pernr
                                    }),
                                    new Filter({
                        ...
                        oModel.read("/UserEventSet", {
                            ...
                            success: function (oDataUserEvents) {
                                ...
                                var sPathToPersonAppointments = "/people/" + aPersons.findIndex(obj => obj.key==oDataUserData.Uname) + "/appointments",
                                    sPathToTeamAppointments = "/appointments",
                                ...
                                for (i = 0; i < oDataUserEvents.results.length; i++) {
                                    ...
                                    if (oDataUserEvents.results[i].Uname != oDataUserData.Uname || oDataUserEvents.results[i].Pernr != oDataUserData.Pernr) { //Logged user is manager. Put team events in "jsonTeamEventsService" model.
                                        aTeamAppointments.push(oUserEvent);
                                    } else {
                                        aPersonAppointments.push(oUserEvent);
                                    }
                                }
                                if (oJsonModel.setProperty(sPathToPersonAppointments, aPersonAppointments) && oJsonTeamEventsModel.setProperty(sPathToTeamAppointments, aTeamAppointments)) {
                                    oJsonModel.refresh();
                                    oJsonTeamEventsModel.refresh();
                                    sap.ui.core.BusyIndicator.hide();
                                } else {
                                    that.showLogInError();
                                }
            ...
            onBeforeRendering: function () { //Fired before every time "ViewMain" view is rendered.
                var that = this,
                    oView = that.getView(),
                    oPlanCaleMain = oView.byId("planCaleMain"),
                    dCurrentDate = new Date();
                //Set view switch buttons width, min, max and start date of "planCaleMain" planning calendar:
                if (oPlanCaleMain) {
                    if (this.getOwnerComponent().byId("container-zfiori_timesheet_3n---app--planCaleMain-Header-ViewSwitch"))
                        this.getOwnerComponent().byId("container-zfiori_timesheet_3n---app--planCaleMain-Header-ViewSwitch").setWidth("350px");
                    if (this.getOwnerComponent().byId("container-zfiori_timesheet_3n---ViewMain--planCaleMain-Header-ViewSwitch"))
                        this.getOwnerComponent().byId("container-zfiori_timesheet_3n---ViewMain--planCaleMain-Header-ViewSwitch").setWidth("350px");
                    if (this.getOwnerComponent().byId("application-ZPS_CAT-display-component---app--planCaleMain-Header-ViewSwitch"))
                        this.getOwnerComponent().byId("application-ZPS_CAT-display-component---app--planCaleMain-Header-ViewSwitch").setWidth("350px");
                    if (this.getOwnerComponent().byId("application-ZPS_CAT-display-component---ViewMain--planCaleMain-Header-ViewSwitch"))
                        this.getOwnerComponent().byId("application-ZPS_CAT-display-component---ViewMain--planCaleMain-Header-ViewSwitch").setWidth("350px");
                    oPlanCaleMain.setStartDate(new Date(dCurrentDate.getFullYear(), dCurrentDate.getMonth(), dCurrentDate.getDate(), 1));
                    oPlanCaleMain.setMinDate(new Date(dCurrentDate.getFullYear(), dCurrentDate.getMonth(), 1));
                    oPlanCaleMain.setMaxDate(new Date(dCurrentDate.getFullYear(), dCurrentDate.getMonth() + 1, 0, 23, 59, 59, 999));
                }
            },
            onPressBtnViewManager: function () { //On press of "btnViewManager" button. Open "viewManager" view.
                var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
                if (oRouter)
                    oRouter.navTo("RouteViewManager", null, true);
            },
            // Gestione primo caricamento (login), evento onBeforeRendering e pulsante per apertura manager view fine
            ...
            formatSpecialDateStartDate: function (oDate) {
                return new Date(oDate.getFullYear(), oDate.getMonth(), oDate.getDate(), 0, 0, 0, 0);
            },
            formatSpecialDateEndDate: function (oDate) {
            ...
            showLogInError: function (oError) { //Show log in error and set "inputNoEditAuthLevel" values to not valid.
                var oView = this.getView(),
                    ...
                    sMessageDetails;
                oView.byId("inputNoEditAuthLevel").setValue("---");
                if (oError) {
                    try {
                    ...
                    MessageBox.error(oI18n.getText("messageBoxLogInError"), {
                        title: oI18n.getText("messageBoxLogInErrorTitle"),
            ...
            _arrangeDialogFragmentCreateWithWbs: function (oRow) {
                ...
                if (!this._pNewAppointmentDialog) {
                    this._pNewAppointmentDialog = Fragment.load({
                        id: oView.getId(),
                        name: "zfioritimesheet3.zfioritimesheet3n.view.FragmentCreazione",
                        controller: this
                    }).then(function (oDialog) {
                        oView.addDependent(oDialog);
                        return oDialog;
            ...
            _arrangeDialogCreateWithWbs: function (oDialog, oRow) {
                ...
                //Check input values and determine begin button enabled state:
            ...
            _setCreateAppointmentDialogContentCreateWithWbs: function (oRow) {
                var oJsonModel = this.getOwnerComponent().getModel("jsonService"),
                    oPlanningCalendar = this.getView().byId("planCaleMain"),
                    ...
                    oPersonSelected = this.byId("dipName"),
                    oActType = this.byId("actType"),
                    oWbsType = this.byId("wbsType"),
                    oWbsLabel = this.byId("wbsTypeLabelCreate"),
                ...
                oWbsLabel.setVisible(true);
                oWbsType.setEditable(true);
                oWbsType.setVisible(true);
            ...
            onPressBtnDays: function (oEvent) {
                var oButton = oEvent.getSource(),
                    oView = this.getView();
                if (!this._pDialog2) {
                    this._pDialog2 = Fragment.load({
                        id: oView.getId(),
                        name: "zfioritimesheet3.zfioritimesheet3n.view.FragmentDays",
                        controller: this
                    }).then(function (oDialog) {
                        oView.addDependent(oDialog);
                        return oDialog;
                ...
                this._pDialog2.then(function (oDialog) {
                    this._configDialogDays(oButton, oDialog);
                    if (oDialog)
                        oDialog.open();
                }.bind(this));
            },
            _configDialogDays: function (oButton, oDialog) {
                // Set responsive padding:
                if (!oButton)
                    oButton = this.byId("btnDays");
                if (!oDialog)
                    oDialog = this.byId("dialogDays");
                var sResponsivePadding = oButton.data("responsivePadding"),
                    sResponsiveStyleClasses = "sapUiResponsivePadding--header sapUiResponsivePadding--subHeader sapUiResponsivePadding--content sapUiResponsivePadding--footer";
                if (sResponsivePadding) {
                    oDialog.addStyleClass(sResponsiveStyleClasses);
                ...
                var oDateFormatter = DateFormat.getDateInstance({
                    ...
                    oListDaysOverview = this.getView().byId("listDialogDays"),
                    oDateRangeWorkdateDays = this.byId("dateRangeWorkdateDialogDays"),
                    oSwitchShowOnlyDaysInError = this.byId("switchShowOnlyDaysInErrorDialogDays"),
                    dCurrentDate = new Date(),
                ...
                //Reset the value state of "dateRangeWorkdateDialogDays" date range filter value:
            ...
            onUpdateFinishedListDialogDays: function (oEvent) {
                //Update the master list object counter after new data is loaded:
                this._updateListItemCountDialogDays(oEvent.getParameter("total"));
            },
            _updateListItemCountDialogDays: function (iTotalItems) {
                var oI18n = this.getOwnerComponent().getModel("i18n").getResourceBundle();
                //Only update the counter if the length is final:
                if (this.byId("listDialogDays").getBinding("items").isLengthFinal())
                    this.byId("labelInfoToolbarDialogDays").setText(oI18n.getText("textDisplaydEntriesDialogDays") + iTotalItems);
            },
            onPressBtnCloseDialogDays: function () {
                // Reset the filter:
                var oBinding = this.byId("listDialogDays").getBinding("items");
                oBinding.filter([]);
                // Close dialog:
                this.byId("dialogDays").close();
            },
            onPressBtnApplySelectedFiltersDialogDays: function () {
                var oListDaysOverview = this.getView().byId("listDialogDays");
                //Set "listDialogDays" list to busy before backend data fetching:
                ...
                oBinding.filter([oFilter]);
                //Set "listDialogDays" list to not busy after backend data fetching:
                oListDaysOverview.setBusy(false);
            },
            onChangeDateRangeWorkdateDialogDays: function () {
                this._validateFilterValuesDialogDays();
            },
            onChangeSwitchShowOnlyDaysInErrorDialogDays: function () {
                this._validateFilterValuesDialogDays();
            },
            _validateFilterValuesDialogDays: function () {
                var oI18n = this.getOwnerComponent().getModel("i18n").getResourceBundle(),
                    oDateRangeWorkdateDays = this.byId("dateRangeWorkdateDialogDays");
                ...
                this._updateEnabledStateBtnApplySelectedFiltersDialogDays();
            },
            _updateEnabledStateBtnApplySelectedFiltersDialogDays: function () {
                var oDateRangeWorkdateDays = this.byId("dateRangeWorkdateDialogDays"),
                    oButtonApplySelectedFilters = this.byId("btnApplySelectedFiltersDialogDays"),
            ...
            handlePlanCaleMainAppoSele: function (oEvent) {
                //Set core to busy before backend data fetching:
                sap.ui.core.BusyIndicator.show();
            ...
            _setDetailsDialogContent: function (oAppointment, oDetailsPopover) {
                var that = this,
                    ...
                    oLabelWbs = this.byId("labelWbsDetails"),
                    ...
                    oJsonModel = this.getOwnerComponent().getModel("jsonService"),
                    oModel = this.getOwnerComponent().getModel("mainService"),
                    sPath = oAppointment.mBindingInfos.key.binding.oContext.sPath,
                    oKey = oJsonModel.getProperty(sPath),
                    sUserEventPath = "UserEventSet(Uname='" + oKey.user + "',Pernr='" + oKey.userPernr + "',Counter='" + oKey.key + "')",
                    oEvent = oModel.oData[sUserEventPath],
                    oStartDateControl,
                    oEndDateControl;
                ...
                if (oEvent.Tasklevel == "ASS001") {
                    oLabelWbs.setVisible(false);
                    oWbs.setVisible(false);
                    ...
                    oLabelWbs.setVisible(true);
                    oWbs.setVisible(true);
            ...
            _removeAppointment: function () {
                ...
                var oCounter = oKeyModel.getData().key,
                    oUser = oJsonModel.getProperty("/people/0");
            ...
            _arrangeDialogFragmentModifica: function () {
                ...
                if (!this._pModAppointmentDialog) {
                    this._pModAppointmentDialog = Fragment.load({
                        id: oView.getId(),
                        name: "zfioritimesheet3.zfioritimesheet3n.view.FragmentModifica",
                        controller: this
                    }).then(function (oDialog) {
                        oView.addDependent(oDialog);
                        return oDialog;
            ...
            _setAppointmentDialogModifica: function () {
                var oJsonModel = this.getOwnerComponent().getModel("jsonService"),
                    oPersonSelected = this.byId("modName"),
                ...
                var oCounter = oKeyModel.getData().key;
                var oUser = oJsonModel.getProperty("/people/0");
                oPersonSelected.setValue(oUser.key);
                ...
                if (oEvent.Tasklevel == "ASS001") {
                    ...
                    oWbsType.setVisible(false);
                    this.byId("wbsTypeLabelModifica").setVisible(false);
                    ...
                    oWbsType.setVisible(true);
                    this.byId("wbsTypeLabelModifica").setVisible(true);
            ...
            handleChangeSelectActivityModifica: function () {
                ...
                if (oRow.Tasklevel == "ASS001") {
                    ...
                    var oSPCStartDate = this.getView().byId("planCaleMain").getStartDate();
            ...
            handleDialogModificaSaveButton: function () {
                var oJsonModel = this.getOwnerComponent().getModel("jsonService"),
                    ...
                    oModel = this.getOwnerComponent().getModel("mainService"),
                    oNewAppointmentDialog = this.byId("modificaDialog"),
                    oNewAppointment;
            ...
            _addNewAppointmentModifica: function (oAppointment) {
                ...
                if (!oEndDate)
                    oEndDate = oWorkDate;
                var oOreLavorate = this.getView().byId("modOreLavorate").getValue();
                if (oRowActValues.Tasklevel == "ASS001") {
            ...
            _setCreateAppointmentDialogContentCreate: function () {
                ...
                var oStartDateControl = DateFormat.getDateTimeInstance({ style: "medium" }).format(oStartDate);
                var oEndDateControl = DateFormat.getDateTimeInstance({ style: "medium" }).format(oEndDate);
                var oWorkdateControl = DateFormat.getDateInstance({ style: "medium" }).format(oStartDate);
                oDateTimePickerStart.setValue(oStartDateControl);
                oDateTimePickerEnd.setValue(oEndDateControl);
                oDatePickerGiornoDate.setValue(oWorkdateControl);
                oTitleInput.setValue("");
                oMoreInfoInput.setValue("");
                oActType.setValue(sDefaultActivityText);
                oActType.setSelectedItem(sDefaultActivity);
            ...
            _ricaricaUserEvent: function () {
                ...
                oModel.read("/UserEventSet", {
                    filters: [sUserEventFilter],
                    success: function (oDataUserEvents) {
                        var sPathToPersonAppointments = "/people/0/appointments",
                            aPersonAppointments = oJsonModel.getProperty(sPathToPersonAppointments),
                            oUserEvent,
                            i;
                        ...
                        for (i = 0; i < oDataUserEvents.results.length; i++) {
                            if (oDataUserEvents.results[i].Uname === oUser.key) {
                                if (oDataUserEvents.results[i].Tasktype == "AS01")
                                        oDataUserEvents.results[i].Catshours = "0.00";
                                var //decEventTotHours = parseFloat(oDataUserEvents.results[i].Catshours),
                                    dEventWorkDate = new Date(oDataUserEvents.results[i].Workdate),
                                    sEventStartDate,
                                    sEventEndDate,
                                    sType;
                                sEventStartDate = that.dateToYyyyMmDd(dEventWorkDate) + "T" + that.msToTime(oDataUserEvents.results[i].Beguz.ms);
            ...
            handleChangeSelectActivityCreate: function (oEvent) {
                var sPath;
                ...
                if (oRow.Tasklevel == "ASS001") {
                    ...
                    oWbsType.setEditable(false);
                    oWbsType.setVisible(false);
                    this.byId("wbsTypeLabelCreate").setVisible(false);
                    ...
                    oWbsType.setEditable(true);
                    oWbsType.setVisible(true);
                    this.byId("wbsTypeLabelCreate").setVisible(true);
                    this.getView().byId("lCreateStartDate").setVisible(false);
            ...
            validateStartEndDateTimePickerCreate: function (oInputStartDate, oInputEndDate) {
                ...
                if (!oInputStartDate.getValue().replace(/\s+/g,"")) {
                    oInputStartDate.setValueState(ValueState.Error);
                    oInputStartDate.setValueStateText(oI18n.getText("validationErrorTextEmptyStartDateTimePicker"));
                } else if (!oInputStartDate._bValid) {
                    oInputStartDate.setValueState(ValueState.Error);
                    oInputStartDate.setValueStateText(oI18n.getText("validationErrorTextInvalidStartDateTimePicker"));
                } else {
                    oInputStartDate.setValueState(ValueState.None);
                }
                if (!oInputEndDate.getValue().replace(/\s+/g,"")) {
                    oInputEndDate.setValueState(ValueState.Error);
                    oInputEndDate.setValueStateText(oI18n.getText("validationErrorTextEmptyEndDateTimePicker"));
                } else if (!oInputEndDate._bValid) {
                    oInputEndDate.setValueState(ValueState.Error);
                    oInputEndDate.setValueStateText(oI18n.getText("validationErrorTextInvalidEndDateTimePicker"));
                } else {
                    oInputEndDate.setValueState(ValueState.None);
                }
                if (oStartDate && oEndDate && oInputStartDate.getValueState() !== ValueState.Error && oInputEndDate.getValueState() !== ValueState.Error) {
                    if (oEndDate.getTime() <= oStartDate.getTime()) {
                        oInputStartDate.setValueState(ValueState.Error);
                        oInputEndDate.setValueState(ValueState.Error);
                        oInputStartDate.setValueStateText(oI18n.getText("validationErrorTextStartEndDateTimePicker"));
                        oInputEndDate.setValueStateText(oI18n.getText("validationErrorTextStartEndDateTimePicker"));
                    } else {
                        oInputStartDate.setValueState(ValueState.None);
                        oInputEndDate.setValueState(ValueState.None);
                    }
            ...
            onChangeStartEndDateInputValuesCreate: function () {
                var oInputStartDate = this.byId("createStartDate"),
                    oInputEndDate = this.byId("createEndDate"),
                    oInputWorkedHours = this.byId("oreLavorate"),
                    flagHours = oInputWorkedHours.getVisible() ? "X" : "";
                if (!flagHours) {
                    this.validateStartEndDateTimePickerCreate(oInputStartDate, oInputEndDate);
                    this.updateButtonEnabledStateAfterInputValidationCreate(this.byId("createDialog"), flagHours);
                }
            },
            ...
            updateButtonEnabledStateAfterInputValidationCreate: function (oDialog, flagHours) {
                var oInputActivity = this.byId("actType"),
                    oInputWbs = this.byId("wbsType"),
                    oInputStartDate = this.byId("createStartDate"),
                    oInputEndDate = this.byId("createEndDate"),
                    oInputWorkdate = this.byId("createGiornoDate"),
            ...
            onChangeStartEndDateInputValuesModifica: function () {
                var oInputStartDate = this.byId("modStartDate"),
                    oInputEndDate = this.byId("modEndDate"),
                    oInputWorkedHours = this.byId("modOreLavorate"),
                    flagHours = oInputWorkedHours.getVisible() ? "X" : "";
                if (!flagHours) {
                    this.validateStartEndDateTimePickerCreate(oInputStartDate, oInputEndDate);
                    this.updateButtonEnabledStateAfterInputValidationModifica(this.byId("modificaDialog"), flagHours);
                }
            },
            ...
            onPlanCaleMainCreate: function (oEvent) {
                var oParam = new sap.ui.model.json.JSONModel();
                oParam.setData(oEvent.getParameters());
                this.getOwnerComponent().setModel(oParam, "paramModel");
                this.onPressBtnAddAppointment(oEvent);
            },
            onPlanCaleMainDragEnter: function (oEvent) {
            ...
            onPlanCaleMainDrop: function (oEvent) {
                //Set core to busy before elements rendering and backend data fetching:
                sap.ui.core.BusyIndicator.show();
                var oAppointment = oEvent.getParameter("appointment"),
                    oStartDate = oEvent.getParameter("startDate"),
                ...
                oModel.callFunction("/ModificaUserEvent", {
                    ...
                    success: function (oReturn) {
                        if (oReturn.message) {
                            ...
                            var oJsonModel = that.getOwnerComponent().getModel("jsonService");
                            oJsonModel.getProperty("/people/").forEach(function (oRiga) {
                                // if (oRiga.key === that.byId("modName").getValue().toString()) {
                                    sPath = "/people/" + i.toString();
                                    sPath += "/appointments";
                                    oPersonAppointments = oJsonModel.getProperty(sPath);
                                    oPersonAppointments.forEach(function (oAppoint) {
            ...
            */
            onPlanCaleMainResize: function (oEvent) {
                //Set core to busy before elements rendering and backend data fetching:
                sap.ui.core.BusyIndicator.show();
                var oAppointment = oEvent.getParameter("appointment"),
